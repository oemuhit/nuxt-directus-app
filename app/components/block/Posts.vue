...
const totalPages = computed(() =>
  Math.ceil((postsData.value?.count || 0) / perPage)
);
    ...
    <div>
          ...
          >
            <div class="relative w-full h-[256px] overflow-hidden rounded-lg">
              <DirectusImage
                v-if="post.image"
                :uuid="
                  typeof post.image === 'string' ? post.image : post.image?.id
                "
                :alt="post.title"
      ...
      <ClientOnly>
        ...
        >
          ...
          >
            <div
              v-if="totalPages > 5 && currentPage > 1"
            ...
            <template v-for="(page, index) in paginationLinks" :key="index">
              ...
              >
                <Button
                  variant="outline"
                  :class="{ 'border-none': currentPage !== page }"
                >
                  {{ page }}
                </Button>
              </PaginationListItem>
